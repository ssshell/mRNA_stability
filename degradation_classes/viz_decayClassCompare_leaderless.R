...
####### compare half-life and pattern degradation class for leaderless genes
...
pattern_CDS_leaderless_logPhase <- read.table("./pattern/CDS_pattern_logPhaseClass.txt",
                                            header = TRUE) %>% 
        # drop_na() %>% 
...
hlAndPattern_logPhase <- inner_join(hl_CDS_leaderless_logPhase, pattern_CDS_leaderless_logPhase, by = "gene",
                                    suffix = c("_HalfLife", "_Pattern"))

hl_CDS_leaderless_logPhase_common <- hl_CDS_leaderless_logPhase[(hl_CDS_leaderless_logPhase$gene %in% hlAndPattern_logPhase$gene), ]
pattern_CDS_leaderless_logPhase_common <- pattern_CDS_leaderless_logPhase[(pattern_CDS_leaderless_logPhase$gene %in% hlAndPattern_logPhase$gene), ]

decay_complete_logPhase <- bind_rows(hl_CDS_leaderless_logPhase_common, pattern_CDS_leaderless_logPhase_common) %>% 
...
upset_logPhase <- upset(fromList(class_msmeg_logPhase_list), nsets = 8, order.by = "freq",
                        ...
                        keep.order = TRUE)
png("./viz_toCompareDecayClass_byHalfLifeAndPattern/CDS_leaderless_decayClass_commonHalfLifeAndPattern_logPhase.png", 
...
####### hypoxia
###### get half-life class
hl_CDS_leaderless_hypoxia <- hl_CDS_complete_class %>% 
...
###### get decay class
pattern_CDS_leaderless_hypoxia <- read.table("./pattern/CDS_pattern_hypoxiaClass.txt",
                                           header = TRUE) %>% 
        # drop_na() %>%
...
###### get genes have both half-life and decay pattern class
hlAndPattern_hypoxia <- inner_join(hl_CDS_leaderless_hypoxia, pattern_CDS_leaderless_hypoxia, by = "gene",
                                   suffix = c("_HalfLife", "_Pattern"))

hl_CDS_leaderless_hypoxia_common <- hl_CDS_leaderless_hypoxia[(hl_CDS_leaderless_hypoxia$gene %in% hlAndPattern_hypoxia$gene), ]
pattern_CDS_leaderless_hypoxia_common <- pattern_CDS_leaderless_hypoxia[(pattern_CDS_leaderless_hypoxia$gene %in% hlAndPattern_hypoxia$gene), ]

decay_complete_hypoxia <- bind_rows(hl_CDS_leaderless_hypoxia_common, pattern_CDS_leaderless_hypoxia_common) %>% 
        mutate(decay_class = factor(decay_class))
...
upset_hypoxia <- upset(fromList(class_msmeg_hypoxia_list), nsets = 8, order.by = "freq",
                       ...
                       keep.order = TRUE)
png("./viz_toCompareDecayClass_byHalfLifeAndPattern/CDS_leaderless_decayClass_commonHalfLifeAndPattern_hypoxia.png", 
    res = 600, width = 5000, height = 3000)
print(upset_hypoxia)
dev.off()
