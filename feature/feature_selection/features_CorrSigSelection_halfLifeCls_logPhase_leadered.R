...
hl_logPhase <- features_leadered_rmZeroVar %>% 
...
features_leadered_corrTarget_tau_p1 <- lapply(hl_logPhase_ordinalCls_features, function(x) cor(x, hl_logPhase_ordinalCls$HalfLife_cls_logPhase_ordinal, method = "kendall"))
...
features_leadered_corr_f1_classSig_hl_logPhase <- features_leadered_corr_f1 %>% 
...
features_leadered_corr_f2_classSig_hl_logPhase <- features_leadered_corr_f2 %>% 
        inner_join(hl_logPhase_classSig, by = 'feature') %>%
        inner_join(features_leadered_corrTarget_tau, by = 'feature') %>% 
        select(-kendall_hl_logPhase) %>%
...
features_leadered_corrSig_hl_logPhase <- bind_cols(features_leadered_corr_f1_classSig_hl_logPhase, features_leadered_corr_f2_classSig_hl_logPhase) %>%
        select(-rs_abs_feature2) %>%
        rename(rs_abs = rs_abs_feature1)

write.table(features_leadered_corrSig_hl_logPhase, "./corrSig_halfLifeCls_logPhase_leadered.txt", quote = FALSE, row.names = FALSE, sep = "\t")
