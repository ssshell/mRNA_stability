...
fc_hypoxiaToLogPhase <- features_leaderless_rmZeroVar %>% 
...
features_leaderless_corrTarget_tau_p1 <- lapply(fc_hypoxiaToLogPhase_ordinalCls_features, function(x) cor(x, fc_hypoxiaToLogPhase_ordinalCls$HalfLife_FCcls_hypoxiaToLogPhase_ordinal, method = "kendall"))
...
features_leaderless_corr_f1_classSig_fc_hypoxiaToLogPhase <- features_leaderless_corr_f1 %>% 
...
features_leaderless_corr_f2_classSig_fc_hypoxiaToLogPhase <- features_leaderless_corr_f2 %>% 
        inner_join(fc_hypoxiaToLogPhase_classSig, by = 'feature') %>%
        inner_join(features_leaderless_corrTarget_tau, by = 'feature') %>% 
        select(-kendall_fc_hypoxiaToLogPhase) %>%
...
features_leaderless_corrSig_fc_hypoxiaToLogPhase <- bind_cols(features_leaderless_corr_f1_classSig_fc_hypoxiaToLogPhase, features_leaderless_corr_f2_classSig_fc_hypoxiaToLogPhase) %>%
        select(-rs_abs_feature2) %>%
        rename(rs_abs = rs_abs_feature1)

write.table(features_leaderless_corrSig_fc_hypoxiaToLogPhase, "./corrSig_halfLifeFcCls_hypoxiaToLogPhase_leaderless.txt", quote = FALSE, row.names = FALSE, sep = "\t")
