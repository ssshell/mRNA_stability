...
path = '../FeatureSet_complete_hypoxia_leaderless/'
files = list.files(path, pattern = '.txt')
setwd('../FeatureSet_complete_hypoxia_leaderless/')
...
###### Other properties
path = '../FeatureSet_otherProperties_hypoxia_leaderless/'
files = list.files(path, pattern = '.txt')
setwd('../FeatureSet_otherProperties_hypoxia_leaderless/')
table_list = lapply(files, read.table, header = TRUE)
...
hl_hypoxia <- features_leaderless_rmZeroVar %>% 
...
features_leaderless_corrTarget_tau_p1 <- lapply(hl_hypoxia_ordinalCls_features, function(x) cor(x, hl_hypoxia_ordinalCls$HalfLife_cls_hypoxia_ordinal, method = "kendall"))
...
features_leaderless_corr_f1_classSig_hl_hypoxia <- features_leaderless_corr_f1 %>% 
...
features_leaderless_corr_f2_classSig_hl_hypoxia <- features_leaderless_corr_f2 %>% 
        inner_join(hl_hypoxia_classSig, by = 'feature') %>%
        inner_join(features_leaderless_corrTarget_tau, by = 'feature') %>% 
        select(-kendall_hl_hypoxia) %>%
...
features_leaderless_corrSig_hl_hypoxia <- bind_cols(features_leaderless_corr_f1_classSig_hl_hypoxia, features_leaderless_corr_f2_classSig_hl_hypoxia) %>%
        select(-rs_abs_feature2) %>%
        rename(rs_abs = rs_abs_feature1)

write.table(features_leaderless_corrSig_hl_hypoxia, "./corrSig_halfLifeCls_hypoxia_leaderless.txt", quote = FALSE, row.names = FALSE, sep = "\t")
